...
OUTPUT_DIR="${OUTPUT_DIR}_${SEED}${FLOWGNN_MODEL_ARG}"
...
if [[ ${TASK} == 'multi_task' ]]; then
  RUN_FN=${WORKDIR}/CodeT5/run_multi_gen.py
  MULTI_TASK_AUG='--max_steps '${16}' --save_steps '${17}' --log_steps '${18}
elif [[ ${TASK} == 'clone' ]]; then
  RUN_FN=${WORKDIR}/CodeT5/run_clone.py
elif [[ ${TASK} == 'defect' ]]; then
  RUN_FN=${WORKDIR}/CodeT5/run_defect.py
else
  RUN_FN=${WORKDIR}/CodeT5/run_gen.py
fi
...
CUDA_VISIBLE_DEVICES=${GPU} \
  ...
  --train_batch_size ${BS} --eval_batch_size ${BS} --gradient_accumulation_steps ${ACCUM} --max_source_length ${SRC_LEN} --max_target_length ${TRG_LEN} --seed ${SEED} --flowgnn_data ${FLOWGNN_MODEL_ARG} \
  2>&1 | tee ${LOG}
