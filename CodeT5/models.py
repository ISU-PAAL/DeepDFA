...
class DefectModel(nn.Module):
    def __init__(self, encoder, config, tokenizer, args, flowgnn_model=None):
        super(DefectModel, self).__init__()
        self.encoder = encoder
        self.config = config
        self.tokenizer = tokenizer
        hidden_size = config.hidden_size
        if flowgnn_model is not None:
            hidden_size += flowgnn_model.out_dim
        self.classifier = nn.Linear(hidden_size, 2)
        self.args = args
        self.flowgnn_model = flowgnn_model
    ...
    def forward(self, source_ids=None, labels=None, graphs=None):
        source_ids = source_ids.view(-1, self.args.max_source_length)
        ...
        if self.flowgnn_model is not None and graphs is not None:
            flowgnn_embed = self.flowgnn_model(graphs, {})
            vec = torch.cat((vec, flowgnn_embed), dim=1)
